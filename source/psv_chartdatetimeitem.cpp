...
PSV_ChartDateTimeItem::PSV_ChartDateTimeItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
{
    return QString("value = %1\n%2").arg(QString::number(y,'f',m_decimal))
...
{
    QString xText = QString("日期:%1").arg(PSV_BEGIN_DATETIME.addSecs((int)dataX).toString("yyyy-MM-dd hh:mm:ss"));
    return xText;
}

...
PSV_VerticalIndicatorItem::PSV_VerticalIndicatorItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
//    m_prevDy = 0;
//    m_prevScaleY = 1;
    m_valueFont.setPointSizeF(m_rect.height() * 0.05);
    adjustMarkTextWidth();//自动会调用adjustRect
//    adjustRect();
    drawBackground();//目前不支持背景修改,默认 黑色
    drawUnderRect();

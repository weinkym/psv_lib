...
//#include "psv_axislistitem.h"

PSV_ChartItem::PSV_ChartItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
void PSV_ChartItem::updateItem()
{
    ...
    {
        m_staStartPoint.setY(m_downAxisItem->validRect().bottom() + m_margin);
    }
    createHVLine();
    afterUpdateItem();
}
    ...
    while(iter.hasNext())
    {
        ...
        if(!m_isStaHidden)
        {
            ...
            if(posX + 2 * m_staMaxWidth > m_chartRect.right())
            {
                ...
                posY += textItem->boundingRect().height() + m_margin;
            }
            else
            {
                posX += m_staMaxWidth;
        ...
        while(iter.hasNext())
        {
            ...
            if(isFound)
            {
                ...
                str.append(QString("<b><font size= 8 color = %1>%2:%3</font><br/></b>")
                           .arg(corStr)
                           .arg(curveName)
                           .arg(QString::number(dataY,'f',m_decimal)));
            }
            else
            {
...
{
    double y = m_margin_total_up + m_rect.top();
    QRectF rect = m_chartRect;
    if(!rect.isValid())
    {
        return y;
...
void PSV_ChartItem::setData(PSV::ATTRIBUTE_ROLE role, const QVariant &value, bool isUpdate)
{
    bool isValib = true;
...
//    PSV_Public::printMes(margin,"margin");
    ...
    qreal y = m_rect.top() + (m_margin_total_up - m_margin_up - m_margin - m_titleItem->boundingRect().height()) * 0.5;
...
//    qreal m_margin = getData(PSV::margin).toDouble();
    ...
    if(!m_isStaHidden && m_curveDataMap.count() > 0)
    {
        qreal height_max = 0;
        qreal width_max = 0;
        QGraphicsTextItem *textItem = new QGraphicsTextItem(this);
        ...
        height_max += m_margin;
        qreal width = m_rect.width() - margin_left - margin_right;
        if(width_max >= width)
        {
...
void PSV_ChartItem::createHVLine()
{
    ...
    m_boundingItem = new QGraphicsRectItem(m_rect,this);
    m_hLineItem->hide();
    m_vLineItem->hide();
    m_boundingItem->hide();
...
{
    QString xText = QString::number(dataX,'f',m_decimal);;
    return xText;
}

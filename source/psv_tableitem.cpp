...
PSV_TableItem::PSV_TableItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
void PSV_TableItem::initVariables(/*const QMap<PSV::PARAM_EKY, QVariant> &m_param*/)
{
    ...
    if(m_param.contains(PSV::dataFont))
    {
        QMap<QString, QVariant> tempMap = m_param.value(PSV::dataFont).toMap();
    ...
    if(m_param.contains(PSV::dataColor))
    {
        QMap<QString, QVariant> tempMap = m_param.value(PSV::dataColor).toMap();
        QMapIterator<QString, QVariant> iter(tempMap);
        while(iter.hasNext())
        {
            iter.next();
    ...
    if(m_param.contains(PSV::vHeaderColor))
    {
        QMap<QString, QVariant> tempMap = m_param.value(PSV::vHeaderColor).toMap();
        QMapIterator<QString, QVariant> iter(tempMap);
        while(iter.hasNext())
        {
            iter.next();

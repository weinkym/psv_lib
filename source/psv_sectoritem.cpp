...
PSV_SectorItem::PSV_SectorItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
void PSV_SectorItem::initVariables(/*const QMap<PSV::PARAM_EKY, QVariant> &m_param*/)
{
    ...
    if(m_param.contains(PSV::height))
    {
        m_height = m_param.value(PSV::height).toDouble();
    }
    if(m_param.contains(PSV::startAngle))
    {
        m_startAngle = m_param.value(PSV::startAngle).toDouble();
    }
    if(m_param.contains(PSV::angle))
    {
        m_angle = m_param.value(PSV::angle).toDouble();
    }
    if(m_param.contains(PSV::color))
    {
        m_color = m_param.value(PSV::color).value<QColor>();
    }
    if(m_param.contains(PSV::factor))
    {
        m_factor = m_param.value(PSV::factor).toInt();
    }
}

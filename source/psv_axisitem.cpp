...
PSV_AxisItem::PSV_AxisItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param,QGraphicsItem *parent)
    ...
    , m_markLength(5)
    , m_arrowsLength(3)
    , m_lineColor(m_defaultColor)
    , m_markColor(m_defaultColor)
{
    m_markFont.setPointSize(12);
    initVariables();    
}

PSV_AxisItem::PSV_AxisItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, const QStringList &list, QGraphicsItem *parent)
    : PSV_Item(param,parent)
    , m_hasArrow(true)
    , m_isList(true)
    , m_isCenter(true)
...
void PSV_AxisItem::initVariables()
{
    ...
    {
        m_maxValue = m_param.value(PSV::maxValue)/*.toDouble()*/;
    }
    if(m_param.contains(PSV::minValue))
    {
        m_minValue = m_param.value(PSV::minValue)/*.toDouble()*/;
    }
    if(m_param.contains(PSV::lineColor))
    {
        m_lineColor = m_param.value(PSV::lineColor).value<QColor>();
    }
    if(m_param.contains(PSV::lineWidth))
    {
        m_lineWidth = m_param.value(PSV::lineWidth).toDouble();
    }
    if(m_param.contains(PSV::markWidth))
    {
        m_markWidth = m_param.value(PSV::markWidth).toDouble();
    }
    if(m_param.contains(PSV::markColor))
    {
        m_markColor = m_param.value(PSV::markColor).value<QColor>();
    }
    if(m_param.contains(PSV::markFont))
    {
        m_markFont = m_param.value(PSV::markFont).value<QFont>();
    }
    if(m_param.contains(PSV::arrowsLength))
    {
        m_arrowsLength = m_param.value(PSV::arrowsLength).toDouble();
    }

    if(m_param.contains(PSV::isCenter))
    {
        m_isCenter = m_param.value(PSV::isCenter).toBool();
    }
    if(m_param.contains(PSV::hasArrow))
    {
        m_hasArrow = m_param.value(PSV::hasArrow).toBool();
    }
//
    if(!m_hasArrow)
    {

...
PSV_ButtonItem::PSV_ButtonItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
void PSV_ButtonItem::updateItem()
{
    ...
    qreal max = qMax(m_rect.width(),m_rect.height());
    QPen pen(Qt::NoPen);
    int zValue = 0;
    {
        ...
        QGraphicsEllipseItem *item =new QGraphicsEllipseItem(m_rect,this);
...
//        lg1.setColorAt(1, QColor(166, 166, 166));
        qreal rate = 0.90;
        QRectF rect(center.x() - m_rect.width() * rate * 0.5
                    ,center.y() - m_rect.height() * rate * 0.5
                    ,rate * m_rect.width(),m_rect.height() * rate);

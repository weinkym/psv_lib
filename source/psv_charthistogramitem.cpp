...
PSV_ChartHistogramItem::PSV_ChartHistogramItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
    ...
    else
    {
        ...
        for(int k = 0; k < m_dataListPair.count(); ++k)
        {
            ...
            for(int i = 0; i < m_dataCount; ++i)
            {
                ...
                if(!m_valueHidden && value > PSV_ZEOR)
                {
...
void PSV_ChartHistogramItem::initHistogramParam()
{
    if(m_param.contains(PSV::valueFont))
    {
        m_valueFont = m_param.value(PSV::valueFont).value<QFont>();
    }    
    if(m_param.contains(PSV::isSameColor))
    {
        m_isSameColor = m_param.value(PSV::isSameColor).toBool();
    }
    if(m_param.contains(PSV::sameColor))
    {
        m_sameColor = m_param.value(PSV::sameColor).value<QColor>();
    }
}
...
QString PSV_ChartHistogramItem::getToolTipText(const double x, const double y)
{
    ...
    return QObject::tr("y = %1\n%2").arg(QString::number(y,'f',m_decimal)).arg(xStr);
}

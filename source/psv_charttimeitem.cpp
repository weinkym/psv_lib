...
PSV_ChartTimeItem::PSV_ChartTimeItem(const QMap<PSV::ATTRIBUTE_ROLE, QVariant> &param, QGraphicsItem *parent)
...
bool PSV_ChartTimeItem::addCurve(const QMap<QTime, double> &data, const PSV_CurveParam &curveParam)
{
 ...
 //======================================
    ...
    textList.append(QString(" %1:(%2,%3)").arg(QString::fromLocal8Bit("最大值")).arg(max_y_pointX.toString("hh:mm:ss")).arg(QString::number(max_y,'f',m_decimal)));
    textList.append(QString(" %1:(%2,%3)").arg(QString::fromLocal8Bit("最小值")).arg(min_y_pointX.toString("hh:mm:ss")).arg(QString::number(min_y,'f',m_decimal)));
...
{
    return QString("value = %1\n时间:%2").arg(QString::number(y,'f',m_decimal))
